/*
 * Name: Theme Base Written by: Okler Themes - (http://www.okler.net) Theme
 * Version: 1.0.0
 */
...
(function($) {
	...
	$items.on('click', function() {
				var prev = $('.nav-main > li.nav-expanded'), next = $(this);
...
(function(theme, $) {
	...
	var $body = $('body'), $html = $('html'), $window = $(window), isAndroid = navigator.userAgent
			.toLowerCase().indexOf('android') > -1;

	// mobile devices with fixed has a lot of issues when focus inputs and
	// others...
	...
	var Skeleton = {
		...
		events : function() {
			...
			if (typeof $.browser !== 'undefined' && !this.customScroll
					&& isAndroid) {
				this.fixScroll();
			}
		...
		fixScroll : function() {
			...
			$window.on('sidebar-left-opened sidebar-right-toggle', function(e,
							data) {
		...
		eventsSidebarLeft : function() {
			...
			var updateNanoScroll = function() {
				if ($.support.transition) {
					$nano.nanoScroller();
					$nano.one('bsTransitionEnd', updateNanoScroll)
		...
		buildSidebarRight : function() {
			this.sidebars.right.isOpened = $html
					.hasClass('sidebar-right-opened');

			if (this.customScroll) {
				this.sidebars.right.$nano = this.sidebars.right.$el
						.find('.nano');
		...
		eventsSidebarRight : function() {
			...
			var close = function(e) {
				if (!!e
						&& !!e.target
						&& ($(e.target).closest('.sidebar-right').get(0) || !$(e.target)
								.closest('html').get(0))) {
					e.preventDefault();
					return false;
				}
			...
			this.sidebars.right.$el.find('.mobile-close').on('click',
					function(e) {
						e.preventDefault();
		...
		eventsContentMenu : function() {
			...
			var close = function(e) {
				if (!!e
						...
						&& ($(e.target).closest('.inner-menu').get(0) || !$(e.target)
								.closest('html').get(0))) {
					return false;
				}
		...
		preventBodyScrollToggle : function(shouldPrevent, $el) {
			setTimeout(function() {
						if (shouldPrevent) {
							$body.data('scrollTop', $body.get(0).scrollTop)
									.css({
												position : 'fixed',
	...
	Mailbox.prototype = {
		...
		eventsCompose : function() {
			...
			var adjustComposeSize = function() {
				...
				contentBodyPaddingBottom = parseInt($contentBody
...
(function($) {
	...
	var LockScreen = {
		...
		show : function() {
			var _self = this, userinfo = this.getUserInfo();
		...
		getUserInfo : function() {
			...
			$info = $('#userbox');
			picture = $info.find('.profile-picture img')
		...
		buildTemplate : function(userinfo) {
			return [
					...
					'</div>', '</section>'].join('').replace(/\{\{picture\}\}/,
					userinfo.picture).replace(/\{\{username\}\}/,
					userinfo.username).replace(/\{\{email\}\}/, userinfo.email);
		}
...
(function($) {
	...
	$(function() {
				$('.panel').on('click', '.panel-actions a.fa-caret-up',
						function(e) {
							e.preventDefault();
							...
							$panel.find('.panel-body, .panel-footer')
									.slideDown(200);
						}).on('click', '.panel-actions a.fa-caret-down',
						function(e) {
							e.preventDefault();
							...
							$panel.find('.panel-body, .panel-footer')
									.slideUp(200);
						}).on('click', '.panel-actions a.fa-times',
						function(e) {
							e.preventDefault();
...
(function(theme, $) {
	...
	PluginMarkdownEditor.prototype = {
		...
		setOptions : function(opts) {
			this.options = $.extend(true, {}, PluginMarkdownEditor.defaults,
					opts);
...
(function(theme, $) {
	...
	PluginTextAreaAutoSize.prototype = {
		...
		setOptions : function(opts) {
			this.options = $.extend(true, {}, PluginTextAreaAutoSize.defaults,
					opts);
	...
	GMapBuilder.prototype = {
		...
		build : function() {
			...
			if (!!window.SnazzyThemes) {
				...
				$.each(window.SnazzyThemes, function(i, theme) {
							themeOpts.push($('<option value="' + theme.id
									+ '">' + theme.name + '</option>').data(
									'json', theme.json));
						});

				this.$wrapper.find('[data-builder-field="maptheme"]')
						.append(themeOpts);
			}
			...
			google.maps.event.addDomListener(window, 'load', function() {
						...
						_self.map = new google.maps.Map(_self.$mapContainer
										.get(0), _self.options.mapOptions);
		...
		mapEvents : function() {
			...
			google.maps.event.addListener(this.map, 'maptypeid_changed',
					function() {
						_self
								.updateControl('maptype', _self.map
												.getMapTypeId());
					});
		...
		updateMapProperty : {
			...
			latlng : function() {
				...
				lat = this.$wrapper
						.find('[data-builder-field][name="latitude"]').val();
				lng = this.$wrapper
						.find('[data-builder-field][name="longitude"]').val();
				...
				if (value === 'false') {
					...
					options = {
						...
						mapTypeControlOptions : {
							style : google.maps.MapTypeControlStyle[value
									.toUpperCase()]
						}
					};
				}
			...
			zoomcontrol : function(value) {
				...
				if (value === 'false') {
					...
					options = {
						zoomControl : true,
						zoomControlOptions : {
							style : google.maps.ZoomControlStyle[value
									.toUpperCase()]
						}
					};
				}
			...
			maptype : function(value) {
				...
				mapStyles = this.$wrapper
						...
						.children('option').filter(':selected').data('json');
			...
			maptheme : function(value) {
				...
				mapType = google.maps.MapTypeId[this.map.getMapTypeId() === 'terrain'
						...
						: 'ROADMAP'];
				options = {};
				json = this.$wrapper.find('[data-builder-field="maptheme"]')
						.children('option').filter(':selected').data('json');
		...
		updateControlValue : {
			...
			latlng : function() {
				...
				this.$wrapper.find('[data-builder-field][name="latitude"]')
						.val(roundNumber(center.lat(), 6));
				this.$wrapper.find('[data-builder-field][name="longitude"]')
			...
			zoomlevel : function() {
				...
				$control = this.$wrapper
						.find('[data-builder-field="zoomlevel"]');

				$control.children('option[value="' + level + '"]').prop(
						'selected', true);
			...
			maptype : function() {
				...
				$control.children('option[value="' + mapType + '"]').prop(
						'selected', true);
			...
			maptheme : function() {
				...
				$control.children('option[value="false"]').prop('selected',
						true);
		...
		editMarker : function(marker) {
			...
			this.marker.$form.find('#MarkerDescription')
		...
		_onGeocodeResult : function(marker, response, status) {
			...
			if (!response || status !== google.maps.GeocoderStatus.OK) {
				...
				marker.lat = Math.round(result.geometry.location.lat()
						* 1000000)
						/ 1000000;
				marker.lng = Math.round(result.geometry.location.lng()
						* 1000000)
						/ 1000000;
		...
		_appendMarkerToList : function(marker) {
			...
			html = html.replace(/\{location\}/, !!marker.title
							? marker.title
							: marker.location);
		...
		_bindMarkerClick : function(marker) {
			...
			html = [
					...
					'{title}', '{description}', '</div>'].join('');

			html = html.replace(/\{title\}/, !!marker.title ? ("<h4>"
							+ marker.title + "</h4>") : "");
			...
			google.maps.event.addListener(marker._instance, 'click',
					function() {
		...
		preview : function() {
			...
			previewHtml = [
					...
					'<div id="'
							+ this.$wrapper
									.find('[data-builder-field="mapid"]').val()
			...
			customScript.text = "window.initialize = function() { "
					+ this.generate() + " init(); }; ";
		...
		getCode : function() {
			this.$getCodeModal.find('.modal-body pre').html(this.generate()
					.replace(/</g, '&lt;').replace(/>/g, '&gt;'));
		},
		...
		generate : function() {
			...
			var zoomControl = this.$wrapper
					.find('[data-builder-field="zoomcontrol"] option:selected')
					.val() !== 'false';
			var mapTypeControl = this.$wrapper
			...
			output = output
					.replace(
							/\{mapid\}/,
							this.$wrapper.find('[data-builder-field="mapid"]')
									.val())
					.replace(
							/\{lat\}/,
							this.$wrapper
			...
			if (this.markers.length > 0) {
				...
				work.push('        var opts = {};')
				work
						.push('        for (var i = 0; i < locations.length; i++) {');
				work
						.push('            opts.position = new google.maps.LatLng( locations[ i ].lat, locations[ i ].lng );');
				work.push('            opts.map = map;');
				work
						.push('            if ( !!locations[ i ] .title ) { opts.title = locations[ i ].title; }');
				work
						.push('            if ( !!locations[ i ] .description ) { opts.description = locations[ i ].description; }');
				work
						.push('            marker = new google.maps.Marker( opts );');
				work.push('');
				work.push('            (function() {');
				work.push('                var html = [');
				work
						.push('                	\'<div style="background-color: #FFF; color: #000; padding: 5px; width: 150px;">\',');
				work.push('                		\'{title}\',');
				...
				work.push('');
				work
						.push('                html = html.replace(/\{title\}/, !!opts.title ?  ("<h4>" + opts.title + "</h4>") : "" );');
				work
						.push('                html = html.replace(/\{description\}/, !!opts.description ?  ("<p>" + opts.description + "</p>") : "" );');

				work
						.push('                var infoWindow = new google.maps.InfoWindow({ content: html });');

				work
						.push('                google.maps.event.addListener( marker, \'click\', function() {');
...
(function(theme, $) {
	...
	PluginCarousel.prototype = {
		...
		build : function() {
			this.options.wrapper.owlCarousel(this.options)
	...
	PluginChartCircular.prototype = {
		...
		build : function() {
			var self = this, $el = this.options.wrapper, value = ($el
					...
					.find('.percent'), shouldAnimate, data;

			shouldAnimate = $.isFunction($.fn['appear'])
	...
	PluginToggle.prototype = {
		...
		events : function($el) {
			...
			$el.find('> label').click(function(e) {
				...
				var $this = $(this), parentSection = $this.parent(), parentWrapper = $this
...
(function(theme, $) {
	...
	WidgetTodoList.prototype = {
		...
		build : function() {
			var _self = this, $check = this.$el.find('.todo-check');
		...
		events : function() {
			var _self = this, $remove = this.$el.find('.todo-remove'), $check = this.$el
					.find('.todo-check'), $window = $(window);
...
(function(theme, $) {
	...
	WidgetToggleExpand.prototype = {
		...
		setOptions : function(opts) {
			this.options = $
					.extend(true, {}, WidgetToggleExpand.defaults, opts);
		...
		events : function() {
			var _self = this, $toggler = this.$el.find('.widget-toggle');

			$toggler.on('click.widget-toggler', function() {
						_self.$el.hasClass('widget-collapsed') ? _self
								.expand(_self.$el) : _self.collapse(_self.$el);
					});
...
(function(theme, $) {
	...
	PluginWordRotate.prototype = {
		...
		build : function() {
			var $el = this.options.wrapper, itemsWrapper = $el
					.find(".word-rotate-items"), items = itemsWrapper
					.find("> span"), firstItem = items.eq(0), firstItemClone = firstItem
					.clone(), itemHeight = firstItem.height(), currentItem = 1, currentTop = 0;
		...
		$.extend(true, $.fn.dataTable.defaults, {
			...
			fnInitComplete : function(settings, json) {
				...
				if ($.isFunction($.fn['select2'])) {
					$('.dataTables_length select', settings.nTableWrapper)
							.select2({
				...
				// search
				$('.dataTables_filter input', settings.nTableWrapper).attr({
							placeholder : 'Search'
