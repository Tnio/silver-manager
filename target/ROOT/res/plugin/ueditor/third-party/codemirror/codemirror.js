...
var CodeMirror = (function() {
	...
	function CodeMirror(place, givenOptions) {
		...
		for (var opt in defaults)
			if (defaults.hasOwnProperty(opt))
				options[opt] = (givenOptions
						&& givenOptions.hasOwnProperty(opt)
						? givenOptions
						: defaults)[opt];
		...
		var instance = wrapper.CodeMirror = {
			...
			indentLine : operation(function(n, dir) {
						if (isLine(n))
							indentLine(n, dir == null ? "smart" : dir
											? "add"
											: "subtract");
					}),
		...
		function handleKeyBinding(e) {
			...
			if (e.metaKey)
				name = "Cmd-" + name;
			if (e.shiftKey
					&& (bound = lookupKey("Shift-" + name, options.extraKeys,
							options.keyMap))) {
				dropShift = true;
			} else {
		...
		function onKeyPress(e) {
			...
			if (options.electricChars && mode.electricChars) {
				var ch = String.fromCharCode(e.charCode == null
						? e.keyCode
						: e.charCode);
		...
		function updateLinesNoUndo(from, to, newText, selFrom, selTo) {
			...
			if (from.ch == 0 && to.ch == 0 && newText[newText.length - 1] == "") {
				...
				if (newText.length == 1)
					...
					lastLine = firstLine.split(to.ch, newText[newText.length
									- 1]);
			...
			if (options.lineWrapping) {
				...
				doc.iter(from.line, from.line + newText.length, function(line) {
							if (line.hidden)
								return;
							var guess = Math.ceil(line.text.length / perLine)
									|| 1;
			...
			doc.iter(from, to, function(line) {
						...
						if (!nextIntact || nextIntact.from > j) {
							if (line.hidden)
								...
								scratch.innerHTML = line.getHTML(ch1, ch2,
										true, tabText);
					...
					function(line) {
						if (line.hidden) {
							...
							if (marker && marker.text)
								text = marker.text.replace("%N%", text != null
												? text
												: "");
							else if (text == null)
								text = "\u00a0";
		...
		function markText(from, to, className) {
			...
			function add(line, from, to, className) {
				getLine(line).addMark(new MarkedText(from, to, className,
						tm.set));
			}
		...
		function setLineHidden(handle, hidden) {
			return changeLine(handle, function(line, no) {
						if (line.hidden != hidden) {
							...
							if (hidden
									&& (sel.from.line == no || sel.to.line == no))
								setSelection(skipHidden(sel.from,
												sel.from.line, sel.from.ch),
										skipHidden(sel.to, sel.to.line,
												sel.to.ch));
		...
		function measureLine(line, ch) {
			...
			if (options.lineWrapping) {
				...
				extra = htmlEscape(line.text.slice(ch + 1, end < 0
								? line.text.length
								: end + (ie ? 5 : 0)));
			}
		...
		function localCoords(pos, inLineWrap) {
			var x, lh = textHeight(), y = lh
					* (heightAtLine(doc, pos.line) - (inLineWrap
							? displayOffset
							: 0));
			if (pos.ch == 0)
				x = 0;
		...
		function matchBrackets(autoclear) {
			var head = sel.inverted ? sel.from : sel.to, line = getLine(head.line), pos = head.ch
					- 1;
			var match = (pos >= 0 && matching[line.text.charAt(pos)])
					|| matching[line.text.charAt(++pos)];
			if (!match)
				return;
			var ch = match.charAt(0), forward = match.charAt(1) == ">", d = forward
					? 1
					: -1, st = line.styles;
			...
			function scan(line, from, to) {
				...
				for (var i = forward ? 0 : st.length - 2, e = forward
						? st.length
						: -2; i != e; i += 2 * d) {
					var text = st[i];
					...
					for (var j = forward ? 0 : text.length - 1, te = forward
							? text.length
							: -1; j != te; j += d, pos += d) {
						if (pos >= from && pos < to
					...
					if (compare) {
						...
						if (changed !== false || !hadState)
							unchanged = 0;
								...
								&& (!mode.indent || mode.indent(hadState, "") == mode
										.indent(state, "")))
							return true;
					}
					++i;
	...
	var commands = CodeMirror.commands = {
		...
		goLineStartSmart : function(cm) {
			...
			var text = cm.getLine(cur.line), firstNonWS = Math.max(0, text
							.search(/\S/));
			cm.setCursor(cur.line, cur.ch <= firstNonWS && cur.ch
							? 0
							: firstNonWS, true);
		},
	...
	CodeMirror.fromTextArea = function(textarea, options) {
		...
		var instance = CodeMirror(function(node) {
					textarea.parentNode
							.insertBefore(node, textarea.nextSibling);
				}, options);
		instance.save = save;
	...
	Line.prototype = {
		...
		replace : function(from, to_, text) {
			var st = [], mk = this.marked, to = to_ == null
					? this.text.length
					: to_;
	...
	var splitLines = "\n\nb".split(/\n/).length != 3 ? function(string) {
		...
		while ((nl = string.indexOf("\n", pos)) > -1) {
			result.push(string.slice(pos, string.charAt(nl - 1) == "\r" ? nl
							- 1 : nl));
			pos = nl + 1;
		}
		...
		return function(type) {
			if (type == "selfcloseTag"
					|| (type == "endTag" && Kludges.autoSelfClosers
							.hasOwnProperty(curState.tagName.toLowerCase())))
				return cont();
			if (type == "endTag") {
...
CodeMirror.defineMode("javascript", function(config, parserConfig) {
	...
	function jsTokenBase(stream, state) {
		...
		if (ch == '"' || ch == "'")
			...
			stream.eatWhile(/[\w\$_]/);
			var word = stream.current(), known = keywords
	...
	function statement(type) {
		...
		if (type == "for")
			return cont(pushlex("form"), expect("("), pushlex(")"), forspec1,
					expect(")"), poplex, statement, poplex);
		if (type == "variable")
			return cont(pushlex("stat"), maybelabel);
		if (type == "switch")
			return cont(pushlex("form"), expression, pushlex("}", "switch"),
					expect("{"), block, poplex, poplex);
		if (type == "case")
			return cont(expression, expect(":"));
		if (type == "default")
			return cont(expect(":"));
		if (type == "catch")
			return cont(pushlex("form"), pushcontext, expect("("), funarg,
					expect(")"), statement, poplex, popcontext);
	...
	function expression(type) {
		...
		if (type == "[")
			return cont(pushlex("]"), commasep(expression, "]"), poplex,
					maybeoperator);
		if (type == "{")
			return cont(pushlex("}"), commasep(objprop, "}"), poplex,
					maybeoperator);
		return cont();
	}
	...
	function functiondef(type, value) {
		...
		if (type == "(")
			return cont(pushlex(")"), pushcontext, commasep(funarg, ")"),
...
CodeMirror.defineMode("css", function(config) {
			...
			return {
				...
				indent : function(state, textAfter) {
					...
					if (/^\}/.test(textAfter))
						n -= state.stack[state.stack.length - 1] == "rule"
								? 2
								: 1;
...
CodeMirror.defineMode("htmlmixed", function(config, parserConfig) {
			...
			function html(stream, state) {
				...
				if (style == "tag" && stream.current() == ">"
					...
					if (/^script$/i.test(state.htmlState.context.tagName)) {
						...
						state.localState = jsMode.startState(htmlMode.indent(
								state.htmlState, ""));
			...
			function javascript(stream, state) {
				...
				return maybeBackup(stream, /<\/\s*script\s*>/, jsMode.token(
								stream, state.localState));
			}
			function css(stream, state) {
				...
				return maybeBackup(stream, /<\/\s*style\s*>/, cssMode.token(
								stream, state.localState));
			}
			...
			return {
				...
				copyState : function(state) {
					if (state.localState)
						var local = CodeMirror.copyState(state.token == css
										? cssMode
										: jsMode, state.localState);
					return {
						token : state.token,
						localState : local,
						mode : state.mode,
