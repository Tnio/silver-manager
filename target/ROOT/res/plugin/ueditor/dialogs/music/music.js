...
(function() {
	...
	Music.prototype = {
		...
		callback : function(data) {
			...
			setTimeout(function() {
						$G('J_resultBar').innerHTML = me
								._renderTemplate(data.song_list);
					}, 300);
		},
		dosearch : function() {
			var me = this;
		...
		listenTest : function(elem) {
			var me = this, view = $G('J_preview'), is_play_action = (elem.className == 'm-try'), old_trying = me
					._getTryingElem();
		...
		_sent : function(param) {
			...
			utils.loadFile(document, {
						src : me.dataUrl + '&query=' + param + '&page_size='
								+ me.total + '&callback=music.callback&.r='
								+ Math.random(),
						tag : "script",
		...
		_getUrl : function(isTryListen) {
			...
			var param = 'from=tiebasongwidget&url=&name='
					...
					+ encodeURIComponent(me
							._removeHtml(selectedItem.album_title))
			...
			for (var i = 0, tmpList; tmpList = data[i++];) {
				...
				for (var j = 0, tmpObj; tmpObj = tmpList[j++];) {
					s.push('<label for="radio-' + i + '-' + j
							+ '" class="m-m">');
					s
							.push('<input type="radio" id="radio-'
									+ i
									+ '-'
									+ j
					...
					s.push('<span class="m-s">' + me._getMaxText(tmpObj.author)
							+ '</span>');
					s.push('<span class="m-z">'
							+ me._getMaxText(tmpObj.album_title) + '</span>');
