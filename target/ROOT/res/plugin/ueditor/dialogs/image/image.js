...
(function() {
	...
	function initAlign() {
		...
		domUtils.on($G("alignIcon"), 'click', function(e) {
					var target = e.target || e.srcElement;
					if (target.className
							&& target.className.indexOf('-align') != -1) {
	...
	RemoteImage.prototype = {
		...
		initEvents : function() {
			...
			domUtils.on($G("width"), 'keyup', function() {
						updatePreview();
						if (locker.checked) {
							var proportion = locker
									.getAttribute('data-proportion');
							$G('height').value = Math.round(this.value
									/ proportion);
						} else {
							_this.updateLocker();
						}
			...
			domUtils.on($G("height"), 'keyup', function() {
						...
						if (locker.checked) {
							var proportion = locker
									.getAttribute('data-proportion');
							$G('width').value = Math.round(this.value
									* proportion);
						} else {
							_this.updateLocker();
						}
		...
		setImage : function(img) {
			...
			var wordImgFlag = img.getAttribute("word_img"), src = wordImgFlag
					? wordImgFlag.replace("&amp;", "&")
		...
		setPreview : function() {
			...
			width = ((!ow || !oh) ? preview.offsetWidth : Math.min(ow,
					preview.offsetWidth));
			width = width + (border * 2) > preview.offsetWidth
					? width
	...
	UploadImage.prototype = {
		...
		initUploader : function() {
			...
			function addFile(file) {
				...
				$btns = $('<div class="file-panel">' + '<span class="cancel">'
						+ lang.uploadDelete + '</span>'
						+ '<span class="rotateRight">' + lang.uploadTurnRight
						+ '</span>' + '<span class="rotateLeft">'
						+ lang.uploadTurnLeft + '</span></div>').appendTo($li), $prgress = $li
	...
	SearchImage.prototype = {
		...
		getImageData : function() {
			var _this = this, key = $G('searchTxt').value, type = $G('searchType').value, keepOriginName = editor.options.keepOriginName
					? "1"
