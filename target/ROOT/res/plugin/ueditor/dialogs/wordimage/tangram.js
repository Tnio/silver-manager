...
/**
 * opera 从10开始不是用opera后面的字符串进行版本的判断 在Browser identification最后添加Version +
 * 数字进行版本标识 opera后面的数字保持在9.80不变
 */
 ...
 * 参数的意思：beforeBegin&lt;span&gt;afterBegin this is span! beforeEnd&lt;/span&gt;
 * afterEnd <br />
...
baidu.flash._Base = (function() {
	...
	return function(options, callBack) {
		 ...
		 *            fnName 调用的函数名
		 * @param {Array}
		 *            params 传入的参数组成的数组,若不许要参数，需传入空数组
		 * @param {Function}
		 *            [callBack] 异步调用后将返回值作为参数的调用回调函数，如无返回值，可以不传入此参数
		 * @return {Null}
		 */
 ...
 * @param {Object}
 *            createOptions 创建flash时需要的参数，请参照baidu.swf.create文档
 ...
 * @param {String}
 *            backgroundUrl 背景图片路径
 * @param {String}
 *            listBacgroundkUrl 布局控件背景
 * @param {String}
 *            buttonUrl 按钮图片不背景
 * @param {String|Function}
 *            selectFileCallback 选择文件的回调
 * @param {String|Function}
 *            exceedFileCallback文件超出限制的最大体积时的回调
 * @param {String|Function}
...
baidu.flash.fileUploader = baidu.flash.fileUploader || function(options) {
	...
	var _flash = new baidu.flash._Base(options, ['selectFile', 'exceedMaxSize',
					'deleteFile', 'uploadStart', 'uploadComplete',
	 ...
	 *            fieldName 上传的表单字段名
	 * @param {Object}
	 *            postData 键值对，上传的POST数据
	 * @param {Number|Array|null|-1}
	 *            [index]上传的文件序列 Int值上传该文件 Array一次串行上传该序列文件 -1/null上传所有文件
	 * @return {Null}
	 */
	 ...
	 *            type {description:String, extention:String}
	 * @return {Null};
	 */
 ...
 */
baidu.sio.callByServer = /** @function */
function(url, callback, opt_options) {
 ...
 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
 * 
 * path: baidu/json/parse.js author: erik, berg version: 1.2 date: 2009/11/23
 */
 ...
 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
 * 
 * path: baidu/json/decode.js author: erik, cat version: 1.3.4 date: 2010/12/23
 */
 ...
 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
 * 
 * path: baidu/json/stringify.js author: erik version: 1.1.0 date: 2010/01/11
 */
 ...
 * Tangram Copyright 2009 Baidu Inc. All rights reserved.
 * 
 * path: baidu/json/encode.js author: erik, cat version: 1.3.4 date: 2010/12/23
 */
