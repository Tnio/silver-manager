/**
 * Created with JetBrains PhpStorm. User: xuheng Date: 12-5-22 Time: 上午11:38 To
...
(function() {
	var canvas = $G("J_brushBoard"), context = canvas.getContext('2d'), drawStep = [], // undo
																						// redo存储
	...
	scrawl.prototype = {
		...
		_addBoardListener : function(saveNum) {
			...
			margin = parseInt(domUtils.getComputedStyle($G("J_wrap"),
					"margin-left"));
			drawStep.push(context.getImageData(0, 0, context.canvas.width,
					context.canvas.height));
			drawStepIndex += 1;

			domUtils.on(canvas, ["mousedown", "mousemove", "mouseup",
							"mouseout"], function(e) {
						...
						switch (e.type) {
							...
							case 'mousemove' :
								...
								var endX = e.clientX - margin, endY = e.clientY
										- margin;
			...
			domUtils.on($G("J_previousStep"), "click", function() {
						if (drawStepIndex > 1) {
							...
							context.clearRect(0, 0, context.canvas.width,
									context.canvas.height);
							context.putImageData(drawStep[drawStepIndex - 1],
									0, 0);
		...
		_addBrushBarListener : function() {
			...
			domUtils.on($G("J_brushBar"), "click", function(e) {
						var target = me.getTarget(e), size = browser.ie
								? target.innerText
								: target.text;
						if (!!size) {
		...
		_addAddImgListener : function() {
			...
			domUtils.on(file, "change", function(e) {
				...
				var target = e.target || e.srcElement, reader = new FileReader();
		...
		_addColorSelect : function(target) {
			var me = this, colorList = $G("J_colorList")
					.getElementsByTagName("td"), eraserList = $G("J_eraserBar").children, brushList = $G("J_brushBar").children;
...
(function() {
	function _appendStyle() {
		var doc = document, head = doc.getElementsByTagName('head')[0], style = doc
				.createElement('style'), cssText = '.scale{visibility:hidden;cursor:move;position:absolute;left:0;top:0;width:100px;height:50px;background-color:#fff;font-size:0;line-height:0;opacity:.4;filter:Alpha(opacity=40);}'
	...
	ScaleBoy.prototype = {
		init : function() {
			...
			domUtils.on(document.body, 'mouseup', function(e) {
						if (me.start) {
							domUtils.un(document.body, 'mousemove',
									me.scaleMousemove);
							if (me.moved) {
		...
		scaleMousemove : function(e) {
			var me = arguments.callee.fp, start = me.start, dir = me.dir
					|| 'def', offset = {
				x : e.clientX - start.x,
				y : e.clientY - start.y
			};
...
function ue_callback(url, state) {
	var doc = document, picBorard = $G("J_picBoard"), img = doc
			.createElement("img");
...
function exec(scrawlObj) {
	if (scrawlObj.isScrawl) {
		...
		if (!!base64) {
				...
				onsuccess : function(xhr) {
					if (!scrawlObj.isCancelScrawl) {
						...
						if (responseObj.state == "SUCCESS") {
							var imgObj = {}, url = editor.options.scrawlUrlPrefix
									+ responseObj.url;
							imgObj.src = url;
							imgObj._src = url;
			...
			var actionUrl = editor.getActionUrl(editor
					.getOpt('scrawlActionName')), params = utils
					.serializeParam(editor.queryCommandValue('serverparam'))
