...
<html>
                            ...
                            <div class="block-content collapse in">
								...
								<div class="well" style="padding-bottom: 20px; margin: 0;">
			                    	...
			                    	<div class="control-group">
										...
										<div class="controls">
											<input type="text" onkeyup="this.value=this.value.replace(/(^\s*)/g,'')" onblur="this.value=this.value.replace(/(\s*$)/g,'')" id="name" name="name" value="${admin.name}" class="validate[required,minSize[4],maxSize[20],custom[notSpecialCharacterAndChinese],ajax[ajaxValidateAdminName]] span3" />
										</div>
									</div>
									<div class="control-group">
										<label class="control-label"><span class="required">*</span>真实姓名</label>
										<div class="controls">
											<input type="text" onkeyup="this.value=this.value.replace(/[^\u4E00-\u9FA5]/g,'')" onblur="this.value=this.value.replace(/[^\u4E00-\u9FA5]/g,'')" id="realName" name="realName" value="${admin.realName}" class="validate[required,minSize[4],maxSize[8],custom[chinese]] text-input span3" />
										</div>
									</div>
									<div class="control-group">
										...
										<div class="controls">
											<select id="roleId" name="role.id" class="validate[required] span3">
		                                        ...
		                                        <c:forEach var="role" items="${roleList}" varStatus="s">
		                                           <option value="${role.id}" <c:if test="${role.id eq admin.role.id}"> selected="selected" </c:if> >${role.name}</option>
		                                       </c:forEach>
		                                	</select>
	                                	</div>
									</div>
	...
	<script type="text/javascript">
		$(function(){
			$('.breadcrumb').html('<li class="active">系统管理&nbsp;/&nbsp;<a href="javascript:quit();">操作员管理</a>&nbsp;/&nbsp;编辑操作员</li>');
