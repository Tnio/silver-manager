/*
 * =========================================================
 * bootstrap-validation.js Original Idea: http:/www.newkou.org (Copyright 2012
 * Stefan Petre) Updated by 不会飞的羊
 * (https://github.com/FateSheep/Validation-for-Bootstrap)
 * =========================================================
 * 
 * Licensed under the (the "License"); you may not use this file except in
 ...
 * the License. =========================================================
 * 2013-4-12 ，由云海飞舞 （yunhaifeiwu修改，为了区别原作品，该组件更名为 bt-validate
 * 
 * =========================================================
 */
!function($) {
	...
	var validateField = function(field, valid) { // 验证字段
		var el = $(field), error = false, errorMsg = '', crule = (el
				...
				.split(' '), msg = (el.attr('btvd-err') == undefined)
				? null
		...
		if (error) {
			var cls = (el.attr('btvd-class') == undefined) ? null : el
					.attr('btvd-class').split(' ');;
			if (len <= 0) {
				el
	...
	var reg = function(obj) {
		$('input, textarea').each(function() {
			var el = $(this), valid = (el.attr('btvd-type') == undefined)
					? null
					: el.attr('btvd-type').split(' ');
			valid1 = (el.attr('btvd-el') == undefined) ? null : el
					.attr('btvd-el').split(' ');
			if (valid != null && valid.length > 0 || valid1 != null
					&& valid1.length > 0) {
