/*
 * Highcharts JS v3.0.6 (2013-10-04) Exporting module
 * 
	...
	p(A.prototype, {
		getSVG : function(c) {
			...
			b = g.exporting.sourceWidth || g.chart.width || /px$/.test(b)
					&& parseInt(b, 10) || 600;
			h = g.exporting.sourceHeight || g.chart.height || /px$/.test(h)
					&& parseInt(h, 10) || 400;
			p(g.chart, {
						animation : !1,
						renderTo : c,
			...
			d.destroy();
			n(c);
			b = b.replace(/zIndex="[^"]+"/g, "").replace(/isShadow="[^"]+"/g,
			...
			return b = b.replace(/(url\(#highcharts-[0-9]+)&quot;/g, "$1")
					.replace(/&quot;/g, "'")
		},
		exportChart : function(c, a) {
			var c = c || {}, d = this.options.exporting, d = this.getSVG(o({
						chart : {
							borderRadius : 0
						}
					}, d.chartOptions, a, {
		...
		addButton : function(c) {
			...
			if (b.enabled !== !1) {
				...
				j ? i = function() {
					...
					a.contextMenu(e.menuClassName, h, e.translateX,
							e.translateY, e.width, e.height, e);
					e.setState(2)
				});
				b.text && b.symbol
		...
		destroyExport : function(c) {
			...
			for (a = 0; a < c.exportSVGElements.length; a++)
				if (d = c.exportSVGElements[a])
					d.onclick = d.ontouchstart = null, c.exportSVGElements[a] = d
							.destroy();
