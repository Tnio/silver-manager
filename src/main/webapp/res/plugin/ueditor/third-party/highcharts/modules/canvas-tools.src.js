 ...
 * @link http://www.phpied.com/rgb-color-parser-in-javascript/ Use it if you
 *       like it
 * 
 */
...
/**
 * @license canvg.js - Javascript SVG parser and renderer on Canvas MIT Licensed
...
/**
 * @license Highcharts JS v3.0.6 (2013-10-04) CanVGRenderer Extension module
 * 
...
(function(Highcharts) { // encapsulate
	...
	extend(CanVGRenderer.prototype, {
		...
		setContainer : function(container, chartWidth, chartHeight) {
			var containerStyle = container.style, containerParent = container.parentNode, containerLeft = containerStyle.left, containerTop = containerStyle.top, containerOffsetWidth = container.offsetWidth, containerOffsetHeight = container.offsetHeight, canvas, initialHiddenStyle = {
				visibility : HIDDEN,
				position : ABSOLUTE
			};
			...
			// the container (and as direct childs to the div specified in the
			// html page)
			...
			// Move away the svg node to a new div inside the container's parent
			// so we can hide it.
		...
		/**
		 * Configures the renderer with the chart. Attach a listener to the
		 * event tooltipRefresh.
		 */
		configure : function(chart) {
			var renderer = this, options = chart.options.tooltip, borderWidth = options.borderWidth, tooltipDiv = renderer.ttDiv, tooltipDivStyle = options.style, tooltipLine = renderer.ttLine, padding = parseInt(
					tooltipDivStyle.padding, 10);
			...
			addEvent(chart, 'tooltipRefresh', function(args) {
				...
				// Compute the best position for the tooltip based on the divs
		...
		/**
		 * Take a color and return it if it's a string, do not make it a
		 * gradient even if it is a gradient. Currently canvg cannot render
		 * gradients (turns out black), see:
		 * http://code.google.com/p/canvg/issues/detail?id=104
		 * 
		 * @param {Object}
		...
		/**
		 * Draws the SVG on the canvas or adds a draw invokation to the deferred
		 * list.
		 */
		draw : function() {
			var renderer = this;
