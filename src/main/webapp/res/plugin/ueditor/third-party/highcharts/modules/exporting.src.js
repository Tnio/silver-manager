/**
 * @license Highcharts JS v3.0.6 (2013-10-04) Exporting module
 * 
	...
	extend(Chart.prototype, {
		...
		getSVG : function(additionalOptions) {
			...
			svg = svg
					...
					// IE specific
					.replace(/<IMG /g, '<image ').replace(/height=([^" ]+)/g,
							'height="$1"').replace(/width=([^" ]+)/g,
							'width="$1"').replace(/hc-svg-href="([^"]+)">/g,
							'xlink:href="$1"/>').replace(/id=([^" >]+)/g,
							'id="$1"')
		...
		contextMenu : function(className, items, x, y, width, height, button) {
			...
			if (!menu) {
				...
				each(items, function(item) {
					if (item) {
						var element = item.separator ? createElement('hr',
								null, null, innerMenu) : createElement(DIV, {
		...
		addButton : function(options) {
			...
			if (onclick) {
				...
				callback = function() {
					chart.contextMenu(button.menuClassName, menuItems,
							button.translateX, button.translateY, button.width,
			...
			if (btnOptions.symbol) {
				symbol = renderer.symbol(btnOptions.symbol,
						...
						btnOptions.symbolY - (symbolSize / 2), symbolSize,
						symbolSize).attr(extend(symbolAttr, {
