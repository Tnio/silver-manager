	...
	<body>
            ...
            <div class="row-fluid">
                ...
                <div class="span9" id="content">
                    ...
                    <form class="form-horizontal" style="margin-bottom: 0;" id="fm" method="post"  enctype="multipart/form-data" onsubmit="return startValidateAndUpload();" action="${pageContext.request.contextPath}/product/save"  autocomplete="off">
						...
						<input name="parent.id" value="${product.parent.id}" type="hidden">
			                  ...
			                  <div class="accordion-body collapse in" id="product-base">
			                    <div class="accordion-inner">
										...
										<div class="row-fluid">
											<div class="span6">
												<div class="control-group">
													...
													<div class="controls"><input type="text" name="parent.name"  value="${product.parent.name}" class="span12" readonly="readonly"/></div>
												</div>
											</div>
											<div class="span6">
												<div class="control-group">
										...
										<div class="control-group" id="innerContent">
											...
											<div class="controls">
												<textarea cols="80" id="productDesc" name="productDesc">${product.productDesc}</textarea>
											</div>
										</div>
										<div class="control-group">
									       <label class="control-label">担保方介绍</label>
									       <div class="controls">
									       	<textarea id="securedPartyDesc" name="securedPartyDesc"  class="span12" onkeyup="this.value=this.value.replace(/(^\s*)/g,'')" onblur="this.value=this.value.replace(/(\s*$)/g,'')">${product.securedPartyDesc}</textarea>
									       </div>
								        </div>
								        <div class="control-group">
									      <label class="control-label">机构介绍</label>
									      <div class="controls">
									      	<textarea id="institutionDesc" name="institutionDesc" rows="2" class="span12" onkeyup="this.value=this.value.replace(/(^\s*)/g,'')" onblur="this.value=this.value.replace(/(\s*$)/g,'')">${product.institutionDesc}</textarea>
									      </div>
								        </div>
								        <div class="control-group" id="innerContent">
											<label class="control-label">债权列表</label>
											<div class="controls">
												<textarea cols="80" id="obligatoryRight" name="obligatoryRight" class="span12" required>${product.obligatoryRight}</textarea>
											</div>
										</div>
										<div class="control-group">
		...
		<script type="text/javascript">
			...
			$(function() {
				...
				if('${operation}' == 'edit'){
			    	 $('.breadcrumb').html('<li class="active">产品管理&nbsp;/&nbsp;<a href="javascript:quit()">活期产品管理</a>&nbsp;/&nbsp;活期产品编辑子产品</li>');
					$('#name').focus();
					$('#save').show();
				}else{
					$('.breadcrumb').html('<li class="active">产品管理&nbsp;/&nbsp;<a href="javascript:quit()">活期产品管理</a>&nbsp;/&nbsp;活期产品查看子产品</li>');
					$('#save').hide();
					readOnly();
				}
			...
			function readOnly(){   
		        ...
		        UE.getEditor('obligatoryRight').addListener("ready", function () {
		        	UE.getEditor('obligatoryRight').setDisabled('fullscreen');
				});
