...
<html>
  ...
  <body background="${pageContext.request.contextPath}/images/backgroud.png" style="margin:0;background-size:cover; ">
   <form id="login" action="${pageContext.request.contextPath}/login" method="post" ><!-- onsubmit="return validate()" -->
	    ...
	    <table width="347" height="336" border="0" align="left" cellpadding="0" cellspacing="0" background="${pageContext.request.contextPath}/images/from_bg.png">
	      ...
	      <tr>
	        <td height="31" colspan="2">
              <label class="uniform">
                <div id="uniform-rememberme" class="checker">
                  <span><input class="uniform_on" id="rememberme" name="rememberme" value="1" type="checkbox">&nbsp;&nbsp;记住用户名</span>
                </div>
              </label>
	        </td>
	      </tr> 
  ...
  <script type="text/javascript">
    ...
    $(function(){
    	...
    	for (var i = 0; i < len; i ++) {
    		if ($.trim(cookieArr[i].split('=')[0]) == 'username') {
    			$('#name').val($.trim(cookieArr[i].split('=')[1]));
    		}
    	}
    	$('#name').css({width:232});
	...
	function checkAdmin() {
		var name = $('#name').val();
		if (name) {
			$.get('${pageContext.request.contextPath}/login/admin', {'name' : name}, function(result){  
		        if (result && result.id > 0 && result.totp == 0) {
		        	location.href = '${pageContext.request.contextPath}/totp/'+name;
		        } else {
