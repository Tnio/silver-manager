/*
 * ! jQuery UI - v1.12.1 - 2016-09-14 http://jqueryui.com Includes: widget.js,
...
(function(factory) {
					...
					this.each(function() {
								...
								if (methodValue !== instance
									...
									returnValue = methodValue
											&& methodValue.jquery
											? returnValue.pushStack(methodValue
													.get())
											: methodValue;
									return false;
								}
		...
		_untrackClassesElement : function(event) {
			...
			$.each(that.classesElementLookup, function(key, value) {
						if ($.inArray(event.target, value) !== -1) {
							that.classesElementLookup[key] = $(value
									.not(event.target).get());
						}
					});
		},
	...
	$.each({
				...
				$.Widget.prototype["_" + method] = function(element, options,
					...
					var hasOptions;
					var effectName = !options ? method : options === true
...
// >>label: Position
	...
	var data = $.extend($.expr[":"], {
				data : $.expr.createPseudo ? $.expr.createPseudo(function(
								dataName) {
	...
	(function() {
		...
		$.extend($.effects, {
			...
			createPlaceholder : function(element) {
				var placeholder, cssPosition = element.css("position"), position = element
						.position();
				...
				element.css({
						...
						}).outerWidth(element.outerWidth()).outerHeight(element
						.outerHeight());
				...
				if (/^(static|relative)/.test(cssPosition)) {
					...
					placeholder = $("<" + element[0].nodeName + ">")
							.insertAfter(element).css({
								...
								// (like img)
								display : /^(inline|ruby)/.test(element
										.css("display"))
										? "inline-block"
										: "block",
								visibility : "hidden",
		...
		$.fn.extend({
			effect : function( /* effect, options, speed, callback */) {
				var args = _normalizeArguments.apply(this, arguments), effectMethod = $.effects.effect[args.effect], defaultMode = effectMethod.mode, queue = args.queue, queueName = queue
						|| "fx", complete = args.complete, mode = args.mode, modes = [], prefilter = function(
						next) {
					var el = $(this), normalizedMode = $.effects.mode(el, mode)
							|| defaultMode;
				...
				function run(next) {
					...
					// as toggle can be either show or hide depending on element
					// state
				...
				return queue === false ? this.each(prefilter).each(run) : this
						.queue(queueName, prefilter).queue(queueName, run);
			},
			...
			cssClip : function(clipObj) {
				if (clipObj) {
					return this.css("clip", "rect(" + clipObj.top + "px "
									+ clipObj.right + "px " + clipObj.bottom
									+ "px " + clipObj.left + "px)");
				}
			...
			transfer : function(options, done) {
				var element = $(this), target = $(options.to), targetFixed = target
		...
		function parseClip(str, element) {
			var outerWidth = element.outerWidth(), outerHeight = element
					.outerHeight(), clipRegex = /^rect\((-?\d*\.?\d*px|-?\d+%|auto),?\s*(-?\d*\.?\d*px|-?\d+%|auto),?\s*(-?\d*\.?\d*px|-?\d+%|auto),?\s*(-?\d*\.?\d*px|-?\d+%|auto)\)$/, values = clipRegex
					.exec(str)
...
/** *************************************************************************** */
	...
	var effectsEffectBlind = $.effects.define("blind", "hide", function(
		...
		var map = {
			...
			right : ["left", "right"]
		}, element = $(this), direction = options.direction || "up", start = element
				.cssClip(), animate = {
			clip : $.extend({}, start)
	...
	var effectsEffectBounce = $.effects.define("bounce",
			function(options, done) {
				...
				// Defaults:
				mode = options.mode, hide = mode === "hide", show = mode === "show", direction = options.direction
						|| "up", distance = options.distance, times = options.times
						|| 5,

				// Number of internal animations
				anims = times * 2 + (show || hide ? 1 : 0), speed = options.duration
						/ anims, easing = options.easing,

				// Utility:
				ref = (direction === "up" || direction === "down")
						? "top"
						: "left", motion = (direction === "up" || direction === "left"), i = 0,
				...
				for (; i < times; i++) {
					...
					element.animate(upAnim, speed, easing).animate(downAnim,
							speed, easing);
	...
	var effectsEffectClip = $.effects.define("clip", "hide", function(options,
		...
		var start, animate = {}, element = $(this), direction = options.direction
				|| "vertical", both = direction === "both", horizontal = both
	...
	var effectsEffectDrop = $.effects.define("drop", "hide", function(options,
		...
		var distance, element = $(this), mode = options.mode, show = mode === "show", direction = options.direction
	...
	// >>group: Effects
	// jscs:disable maximumLineLength
	// >>description: Explodes an element in all directions into n pieces.
	// Implodes an element to its original wholeness.
	// jscs:enable maximumLineLength
	...
	var effectsEffectExplode = $.effects.define("explode", "hide", function(
		...
		var i, j, left, top, mx, my, rows = options.pieces ? Math.round(Math
				.sqrt(options.pieces)) : 3, cells = rows, element = $(this), mode = options.mode, show = mode === "show",

		// Show and then visibility:hidden the element before calculating offset
		offset = element.show().css("visibility", "hidden").offset(),

		// Width and height of a piece
		width = Math.ceil(element.outerWidth() / cells), height = Math
				.ceil(element.outerHeight() / rows), pieces = [];
			...
			for (j = 0; j < cells; j++) { // |||
				...
				element.clone().appendTo("body").wrap("<div></div>").css({
						...
						.parent().addClass("ui-effects-explode").css({
									...
									opacity : show ? 1 : 0
								}, options.duration || 500, options.easing,
								childComplete);
			}
		}
	...
	var effectsEffectFold = $.effects.define("fold", "hide", function(options,
			done) {

		// Create element
		var element = $(this), mode = options.mode, show = mode === "show", hide = mode === "hide", size = options.size
				|| 15, percent = /([0-9]+)%/.exec(size), horizFirst = !!options.horizFirst, ref = horizFirst
				? ["right", "bottom"]
		...
		element.queue(function(next) {
			if (placeholder) {
				placeholder.animate($.effects.clipToBox(animation1), duration,
						options.easing).animate(
						$.effects.clipToBox(animation2), duration,
						options.easing);
			}

			next();
		}).animate(animation1, duration, options.easing).animate(animation2,
				duration, options.easing).queue(done);
	...
	// >>group: Effects
	// >>description: Highlights the background of an element in a defined color
	...
	var effectsEffectSize = $.effects.define("size", function(options, done) {
		...
		// Copy for children
		cProps = ["fontSize"], vProps = ["borderTopWidth", "borderBottomWidth",
				"paddingTop", "paddingBottom"], hProps = ["borderLeftWidth",
				"borderRightWidth", "paddingLeft", "paddingRight"],

		// Set options
		mode = options.mode, restore = mode !== "effect", scale = options.scale
				|| "both", origin = options.origin || ["middle", "center"], position = element
				.css("position"), pos = element.position(), original = $.effects
				.scaledDimensions(element), from = options.from || original, to = options.to
		...
		if (scale === "box" || scale === "both") {
			...
			if (factor.from.y !== factor.to.y) {
				from = $.effects.setTransition(element, vProps, factor.from.y,
						from);
			...
			if (factor.from.x !== factor.to.x) {
				from = $.effects.setTransition(element, hProps, factor.from.x,
						from);
		...
		if (scale === "content" || scale === "both") {
			...
			// Vertical props scaling
			if (factor.from.y !== factor.to.y) {
				from = $.effects.setTransition(element, cProps, factor.from.y,
						from);
		...
		if (origin) {
			...
			from.left = (original.outerWidth - from.outerWidth) * baseline.x
					+ pos.left;
			to.top = (original.outerHeight - to.outerHeight) * baseline.y
					+ pos.top;
			to.left = (original.outerWidth - to.outerWidth) * baseline.x
					+ pos.left;
		}
		element.css(from);
		...
		if (scale === "content" || scale === "both") {
			...
			element.find("*[width]").each(function() {
				var child = $(this), childOriginal = $.effects
						.scaledDimensions(child), childFrom = {
				...
				// Vertical props scaling
				if (factor.from.y !== factor.to.y) {
					childFrom = $.effects.setTransition(child, vProps,
							factor.from.y, childFrom);
					childTo = $.effects.setTransition(child, vProps,
							factor.to.y, childTo);
				}

				// Horizontal props scaling
				if (factor.from.x !== factor.to.x) {
					childFrom = $.effects.setTransition(child, hProps,
							factor.from.x, childFrom);
					childTo = $.effects.setTransition(child, hProps,
							factor.to.x, childTo);
				}
	...
	var effectsEffectScale = $.effects.define("scale", function(options, done) {
				...
				// Create element
				var el = $(this), mode = options.mode, percent = parseInt(
						options.percent, 10)
	...
	// >>label: Puff Effect
	// >>group: Effects
	// >>description: Creates a puff effect by scaling the element up and hiding
	// it at the same time.
	// >>docs: http://api.jqueryui.com/puff-effect/
	// >>demos: http://jqueryui.com/effect/

	var effectsEffectPuff = $.effects.define("puff", "hide", function(options,
					done) {
	...
	var effectsEffectPulsate = $.effects.define("pulsate", "show", function(
			options, done) {
		var element = $(this), mode = options.mode, show = mode === "show", hide = mode === "hide", showhide = show
				|| hide,

		// Showing or hiding leaves off the "last" animation
		anims = ((options.times || 5) * 2) + (showhide ? 1 : 0), duration = options.duration
				/ anims, animateTo = 0, i = 1, queuelen = element.queue().length;
	...
	var effectsEffectShake = $.effects.define("shake", function(options, done) {
		...
		var i = 1, element = $(this), direction = options.direction || "left", distance = options.distance
				|| 20, times = options.times || 3, anims = times * 2 + 1, speed = Math
				.round(options.duration / anims), ref = (direction === "up" || direction === "down")
				? "top"
		...
		// Shakes
		for (; i < times; i++) {
			element.animate(animation1, speed, options.easing).animate(
					animation2, speed, options.easing);
		}

		element.animate(animation1, speed, options.easing).animate(animation,
				speed / 2, options.easing).queue(done);
	...
	var effectsEffectSlide = $.effects.define("slide", "show", function(
		...
		var startClip, startRef, element = $(this), map = {
			...
			left : ["right", "left"],
			right : ["left", "right"]
		}, mode = options.mode, direction = options.direction || "left", ref = (direction === "up" || direction === "down")
				? "top"
				: "left", positiveMotion = (direction === "up" || direction === "left"), distance = options.distance
	...
	$.ui.focusable = function(element, hasTabindex) {
		var map, mapName, img, focusableIfVisible, fieldset, nodeName = element.nodeName
				.toLowerCase();
		...
		if (/^(input|select|textarea|button|object)$/.test(nodeName)) {
			...
			if (focusableIfVisible) {
				...
				// disabled.
				// Since controls generally aren't placed inside legends, we
				// skip
	...
	var scrollParent = $.fn.scrollParent = function(includeHidden) {
		var position = this.css("position"), excludeStaticParent = position === "absolute", overflowRegex = includeHidden
				? /(auto|scroll|hidden)/
	...
	var widgetsAccordion = $.widget("ui.accordion", {
		...
		_createIcons : function() {
			...
			if (icons) {
				...
				this._removeClass(children, icons.header)._addClass(children,
						null, icons.activeHeader)._addClass(this.headers,
						"ui-accordion-icons");
			}
		},
		...
		_destroy : function() {
			...
			contents = this.headers.next().css("display", "")
		...
		_refresh : function() {
			...
			this._addClass(this.active, "ui-accordion-header-active",
					"ui-state-active")._removeClass(this.active,
		...
		_eventHandler : function(event) {
			...
			if (options.icons) {
				...
				this._removeClass(activeChildren, null,
						options.icons.activeHeader)._addClass(activeChildren,
			...
			if (!clickedIsActive) {
				this._removeClass(clicked, "ui-accordion-header-collapsed")
						._addClass(clicked, "ui-accordion-header-active",
								"ui-state-active");
				if (options.icons) {
					clickedChildren = clicked
							.children(".ui-accordion-header-icon");
					this._removeClass(clickedChildren, null,
							options.icons.header)._addClass(clickedChildren,
		...
		_toggleComplete : function(data) {
			...
			this._removeClass(prev, "ui-accordion-header-active")._addClass(
					prev, "ui-accordion-header-collapsed");
	...
	var widgetsMenu = $.widget("ui.menu", {
		...
		_destroy : function() {
			var items = this.element.find(".ui-menu-item")
					.removeAttr("role aria-disabled"), submenus = items
					.children(".ui-menu-item-wrapper").removeUniqueId()
		...
		refresh : function() {
			var menus, items, newSubmenus, newItems, newWrappers, that = this, icon = this.options.icons.submenu, submenus = this.element
			...
			newSubmenus = submenus.filter(":not(.ui-menu)").hide().attr({
				...
				var menu = $(this), item = menu.prev(), submenuCaret = $("<span>")
			...
			items.not(".ui-menu-item").each(function() {
				...
				if (that._isDivider(item)) {
					that
							._addClass(item, "ui-menu-divider",
									"ui-widget-content");
				}
			});
			...
			this._addClass(newItems, "ui-menu-item")._addClass(newWrappers,
	...
	$.widget("ui.autocomplete", {
		...
		_create : function() {
			...
			this
					._addClass(this.liveRegion, null,
		...
		_destroy : function() {
			...
			if (this.options.items.controlgroupLabel) {
				this.element.find(this.options.items.controlgroupLabel)
						.find(".ui-controlgroup-label-contents").contents()
						.unwrap();
			}
		},
				...
				that.element.find(selector).each(function() {
					...
					if (instance) {
						instanceOptions.classes = that._resolveClassesValues(
								instanceOptions.classes, instance);
					}
					...
					$.data(widgetElement[0], "ui-controlgroup-data", instance
									? instance
	...
	$.widget("ui.checkboxradio", [$.ui.formResetMixin, {
		...
		_getCreateOptions : function() {
			...
			this.label.contents().not(this.element[0]).each(function() {
				...
				// each element to get a
				// string representation of the label, without the input as part
				// of it.
		...
		_toggleClasses : function() {
			...
			if (this.options.icon && this.type === "checkbox") {
				this._toggleClass(this.icon, null,
						"ui-icon-check ui-state-checked", checked)
						._toggleClass(this.icon, null, "ui-icon-blank",
								!checked);
			}
			...
			if (this.options.icon) {
				...
				if (this.type === "checkbox") {
					toAdd += checked
							...
							: "ui-icon-blank";
					this._removeClass(this.icon, null, checked
									? "ui-icon-blank"
									: "ui-icon-check");
				} else {
					toAdd += "ui-icon-blank";
	...
	$.widget("ui.button", {
		...
		_updateIcon : function(option, value) {
			var icon = option !== "iconPosition", position = icon
					? this.options.iconPosition
					: value, displayBlock = position === "top"
					|| position === "bottom";
		...
		_attachIcon : function(iconPosition) {
			this.element[/^(?:end|bottom)/.test(iconPosition)
					? "append"
					: "prepend"](this.icon);
		},
		...
		_setOptions : function(options) {
			var newShowLabel = options.showLabel === undefined
					...
					: options.showLabel, newIcon = options.icon === undefined
					? this.options.icon
					: options.icon;
	...
	if ($.uiBackCompat !== false) {
		...
		$.fn.button = (function(orig) {
			return function() {
				if (!this.length
						...
						|| (this.length && this[0].tagName === "INPUT" && (this
		...
		$.fn.buttonset = function() {
			...
			if (arguments[0] === "option" && arguments[1] === "items"
					&& arguments[2]) {
				return this.controlgroup.apply(this, [arguments[0],
	...
	$.extend(Datepicker.prototype, {
		...
		_destroyDatepicker : function(target) {
			...
			if (nodeName === "input") {
				...
				$target.removeClass(this.markerClassName).off("focus",
						this._showDatepicker).off("keydown", this._doKeyDown)
						.off("keypress", this._doKeyPress).off("keyup",
								this._doKeyUp);
			...
			getNumber = function(match) {
				var isDoubled = lookAhead(match), size = (match === "@"
						...
						: (match === "!" ? 20 : (match === "y" && isDoubled
								? 4
								: (match === "o" ? 3 : 2)))), minSize = (match === "y"
						? size
						: 1), digits = new RegExp("^\\d{" + minSize + ","
		...
		_adjustInstDate : function(inst, offset, period) {
			var year = inst.selectedYear + (period === "Y" ? offset : 0), month = inst.selectedMonth
	...
	$.widget("ui.draggable", $.ui.mouse, {
		...
		_blockFrames : function(selector) {
			this.iframeBlocks = this.document.find(selector).map(function() {
				...
				return $("<div>").css("position", "absolute").appendTo(iframe
						.parent()).outerWidth(iframe.outerWidth())
						.outerHeight(iframe.outerHeight()).offset(iframe
								.offset())[0];
			});
		},
		...
		_setHandleClassName : function() {
			this.handleElement = this.options.handle ? this.element
					.find(this.options.handle) : this.element;
		...
		_createHelper : function(event) {
			...
			var o = this.options, helperIsFunction = $.isFunction(o.helper), helper = helperIsFunction
					...
					: (o.helper === "clone" ? this.element.clone()
		...
		_getParentOffset : function() {
			...
			// the scroll parent isn't
			// the document, which means that the scroll is included in the
		...
		_convertPositionTo : function(d, pos) {
			...
			return {
				...
				pos.top +
						...
						// border)
						this.offset.parent.top * mod - ((this.cssPosition === "fixed"
				...
				pos.left +
						...
						// The offsetParent's offset without borders (offset +
						// border)
						this.offset.parent.left * mod - ((this.cssPosition === "fixed"
				...
				if (innermostIntersecting) {
					...
					if (!sortable.isOver) {
						...
						sortable.currentItem = ui.helper
								.appendTo(sortable.element).data(
					...
					if (sortable.isOver) {
						...
						sortable._trigger("out", event, sortable
										._uiHash(sortable));
	...
	$.widget("ui.resizable", $.ui.mouse, {
		...
		_destroy : function() {
			...
			var wrapper, _destroy = function(exp) {
				$(exp).removeData("resizable").removeData("ui-resizable")
						.off(".resizable").find(".ui-resizable-handle")
						.remove();
			};
		...
		_setupHandles : function() {
			...
			this._handles = this._handles.add(this.element
		...
		_mouseDrag : function(event) {
			...
			var data, props, smp = this.originalMousePosition, a = this.axis, dx = (event.pageX - smp.left)
		...
		_mouseStop : function(event) {
			...
			if (this._helper) {
				...
				left = (parseFloat(that.element.css("left")) + (that.position.left - that.originalPosition.left))
						|| null;
				top = (parseFloat(that.element.css("top")) + (that.position.top - that.originalPosition.top))
						|| null;
		...
		_respectSize : function(data) {
			...
			var o = this._vBoundaries, a = this.axis, ismaxw = this
					...
					+ this.originalSize.width, dh = this.originalPosition.top
					+ this.originalSize.height, cw = /sw|nw|w/.test(a), ch = /nw|ne|n/
					.test(a);
			if (isminw) {
		...
		_getPaddingPlusBorderDimensions : function(element) {
			var i = 0, widths = [], borders = [element.css("borderTopWidth"),
					...
					element.css("paddingTop"), element.css("paddingRight"),
					element.css("paddingBottom"), element.css("paddingLeft")];
		...
		stop : function(event) {
			var that = $(this).resizable("instance"), o = that.options, pr = that._proportionallyResizeElements, ista = pr.length
				...
				height : (that.size.height - soffseth)
					...
					|| null, top = (parseFloat(that.element.css("top")) + (that.position.top - that.originalPosition.top))
					|| null;
		...
		resize : function(event, ui) {
			...
			$(o.alsoResize).each(function() {
				var el = $(this), start = $(this)
						.data("ui-resizable-alsoresize"), style = {}, css = el
						.parents(ui.originalElement[0]).length ? ["width",
	...
	$.widget("ui.dialog", {
		...
		_createTitlebar : function() {
			...
			this.uiDialogTitlebarClose = $("<button type='button'></button>")
					.button({
								label : $("<a>").text(this.options.closeText)
										.html(),
		...
		_createButtons : function() {
			...
			$.each(buttons, function(name, props) {
						...
						$("<button></button>", props).button(buttonOptions)
								.appendTo(that.uiButtonSet).on("click",
										function() {
		...
		drag : function(draggable, event) {
			...
			$.each($.ui.ddmanager.droppables[draggable.options.scope] || [],
					function() {
						...
						var parentInstance, scope, parent, intersects = intersect(
								draggable, this, this.options.tolerance, event), c = !intersects
	...
	var widgetsProgressbar = $.widget("ui.progressbar", {
		...
		_refreshValue : function() {
			...
			this._toggleClass(this.valueDiv, "ui-progressbar-complete", null,
					value === this.options.max)._toggleClass(
		...
		_mouseDrag : function(event) {
			...
			this.selectees.each(function() {
				...
				if (options.tolerance === "touch") {
					hit = (!(offset.left > x2 || offset.right < x1
							|| offset.top > y2 || offset.bottom < y1));
				} else if (options.tolerance === "fit") {
					hit = (offset.left > x1 && offset.right < x2
							&& offset.top > y1 && offset.bottom < y2);
				}
	...
	// far beyond the limitations of a native select.
	// jscs:enable maximumLineLength
	...
	var widgetsSelectmenu = $.widget("ui.selectmenu", [$.ui.formResetMixin, {
		...
		_drawButton : function() {
			var icon, that = this, item = this._parseOption(this.element
			...
			this._addClass(icon, "ui-selectmenu-icon", "ui-icon "
		...
		refresh : function() {
			this._refreshMenu();
			this.buttonItem.replaceWith(this.buttonItem = this
							._renderButtonItem(

									// Fall back to an empty object in case
		...
		_renderMenu : function(ul, items) {
			...
			$.each(items, function(index, item) {
						...
						if (item.optgroup !== currentOptgroup) {
							...
							that._addClass(li, "ui-selectmenu-optgroup",
									"ui-menu-divider"
		...
		_toggleAttr : function() {
			...
			// make sure
			// we always remove classes first and add them second, otherwise if
	...
	var widgetsSlider = $.widget("ui.slider", $.ui.mouse, {
		...
		_createHandles : function() {
			var i, handleCount, options = this.options, existingHandles = this.element
					.find(".ui-slider-handle"), handle = "<span tabindex='0'></span>", handles = [];
			...
			this
					._addClass(this.handles, "ui-slider-handle",
							"ui-state-default");

			this.handle = this.handles.eq(0);

			this.handles.each(function(i) {
						$(this).data("ui-slider-handle-index", i).attr(
								"tabIndex", 0);
					});
		},
		...
		_slide : function(event, index, newVal) {
			var allowed, otherVal, currentValue = this.value(), newValues = this
					.values();
			...
			if (this._hasMultipleValues()) {
				...
				if (this.options.values.length === 2
					...
					newVal = index === 0 ? Math.min(otherVal, newVal) : Math
							.max(otherVal, newVal);
				}
			...
			allowed = this._trigger("slide", event, this._uiHash(index, newVal,
							newValues));
				...
				switch (event.keyCode) {
					...
					case $.ui.keyCode.LEFT :
						...
						if (!this._keySliding) {
							...
							this._addClass($(event.target), null,
									"ui-state-active");
							allowed = this._start(event, index);
							if (allowed === false) {
								return;
			...
			if (this.options.scroll) {
				if (this.scrollParent[0] !== this.document[0]
					...
					if (event.pageY - this.document.scrollTop() < o.scrollSensitivity) {
						scrolled = this.document.scrollTop(this.document
								.scrollTop()
								- o.scrollSpeed);
					...
					if (event.pageX - this.document.scrollLeft() < o.scrollSensitivity) {
						scrolled = this.document.scrollLeft(this.document
										.scrollLeft()
										- o.scrollSpeed);
			...
			if (!o.placeholder || o.placeholder.constructor === String) {
				...
				o.placeholder = {
					element : function() {
						...
						if (nodeName === "tbody") {
							that
									._createTrPlaceholder(that.currentItem
													.find("tr").eq(0), $(
													"<tr>", that.document[0])
													.appendTo(element));
						} else if (nodeName === "tr") {
					...
					update : function(container, p) {
						...
						if (!p.height()) {
							p
									.height(that.currentItem.innerHeight()
													- parseInt(
															that.currentItem
																	.css("paddingTop")
																	|| 0, 10)
						...
						if (!p.width()) {
							p
									.width(	that.currentItem.innerWidth()
													- parseInt(
															that.currentItem
																	.css("paddingLeft")
																	|| 0, 10)
		...
		_createTrPlaceholder : function(sourceTr, targetTr) {
			...
			sourceTr.children().each(function() {
				$("<td>&#160;</td>", that.document[0]).attr("colspan",
						$(this).attr("colspan") || 1).appendTo(targetTr);
			});
		},
	...
	$.widget("ui.spinner", {
		...
		_events : {
			...
			"mousedown .ui-spinner-button" : function(event) {
				...
				// spinning.
				previous = this.element[0] === $.ui
						.safeActiveElement(this.document[0])
						? this.previous
						: this.element.val();
				function checkFocus() {
					var isActive = this.element[0] === $.ui
							.safeActiveElement(this.document[0]);
					if (!isActive) {
						this.element.trigger("focus");
						this.previous = previous;
		...
		_draw : function() {
			...
			// Button bindings
			this.buttons = this.uiSpinner.children("a").attr("tabIndex", -1)
			...
			if (this.buttons.height() > Math
					.ceil(this.uiSpinner.height() * 0.5)
		...
		_setOptionDisabled : function(value) {
			...
			this._toggleClass(this.uiSpinner, null, "ui-state-disabled",
					!!value);
			this.element.prop("disabled", !!value);
		...
		$.widget("ui.spinner", $.ui.spinner, {
					_enhance : function() {
						this.uiSpinner = this.element.attr("autocomplete",
								"off").wrap(this._uiSpinnerHtml()).parent()
	...
	$.widget("ui.tabs", {
		...
		_tabKeydown : function(event) {
			var focusedTab = $($.ui.safeActiveElement(this.document[0]))
					.closest("li"), selectedIndex = this.tabs.index(focusedTab), goingForward = true;
		...
		_refresh : function() {
			...
			if (!this.active.length) {
				...
				this
						._addClass(this.active, "ui-tabs-active",
								"ui-state-active");
	...
	$.widget("ui.tooltip", {
		...
		_disable : function() {
			...
			this.disabledTitles = this.disabledTitles.add(this.element
							.find(this.options.items).addBack().filter(
									function() {
										var element = $(this);
										if (element.is("[title]")) {
											return element.data(
		...
		_updateContent : function(target, event) {
			...
			content = contentOption.call(target[0], function(response) {
						...
						that._delay(function() {
									...
									// Ignore async response if tooltip was
									// closed already
		...
		_open : function(event, target, content) {
			...
			// from the beginning
			this.liveRegion.children().hide();
			a11yContent = $("<div>").html(tooltip.find(".ui-tooltip-content")
					.html());
			...
			if (this.options.track && this.options.show
		...
		close : function(event) {
			var tooltip, that = this, target = $(event
					? event.currentTarget
					: this.element), tooltipData = this._find(target);
		...
		_tooltip : function(element) {
			var tooltip = $("<div>").attr("role", "tooltip"), content = $("<div>")
					.appendTo(tooltip), id = tooltip.uniqueId().attr("id");

			this._addClass(content, "ui-tooltip-content");
			this
					._addClass(tooltip, "ui-tooltip",
							"ui-widget ui-widget-content");
